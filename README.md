...
- создание, чтение, редактирование и сравнение документов через агента;
- версии документов: цепочка из файла-источника и изменённых копий;
- сравнение версий документов (построчный diff со статистикой изменений);
- скачивание исходных файлов и созданных документов;
- меню действий по документу/чату («⋯»): скачать, сравнить, удалить, переименовать;
...
- адаптивная вёрстка: компактная мобильная загрузка и drawer-навигация;
- Docker Compose;
...
Агент работает через GigaChat-native function calling и может вызывать шесть инструментов:

| Инструмент | Назначение |
|---|---|
| `search_documents` | Семантический + keyword поиск по документам пользователя с возвратом релевантных фрагментов |
| `read_document` | Чтение извлечённого текста документа по id (с поддержкой окна `offset` для длинных файлов) |
| `list_documents` | Перечисление всех документов пользователя |
| `create_document` | Генерация структурированного документа (договор, отчёт, инструкция) из найденных данных |
| `edit_document` | Изменение существующего документа с сохранением версии |
| `compare_documents` | Построчное сравнение двух документов или двух версий одного документа |

Документы, созданные или изменённые агентом, попадают в «цепочку версий»: у новой версии заполняется `source_file_id`, указывающий на исходный документ, а версия с `content` — это логическое «состояние» на момент сравнения. Агент никогда не выдумывает id документов: запрошенный документ ищется по `original_filename` среди реальных файлов пользователя, а отсутствующие ссылки честно докладываются пользователю.
...
| GET | `/api/documents/{id}/file` | Скачивание исходного файла |
| GET | `/api/documents/{id}/versions` | Цепочка версий документа |
| POST | `/api/documents/{id}/index` | Индексация |
| POST | `/api/documents/compare` | Построчное сравнение двух документов/версий |
| DELETE | `/api/documents/{id}` | Удаление документа |
| DELETE | `/api/documents` | Удаление всех документов |
| POST | `/api/search` | Hybrid search |
| POST | `/api/chat` | RAG-вопрос |
| POST | `/api/agent` | AI-агент (один ответ) |
| POST | `/api/agent/stream` | Агент с SSE-потоком шагов |
| GET | `/api/chats` | История чатов |
| POST | `/api/chats` | Создание чата |
| GET | `/api/chats/{id}/messages` | Сообщения чата |
| PATCH | `/api/chats/{id}` | Переименование чата |
| DELETE | `/api/chats/{id}` | Удаление чата |
| POST | `/api/reports` | Пожаловаться на ответ агента |
...
| GET | `/api/admin/users/{id}/reports` | Жалобы на пользователя |

## Сравнение документов

Пользователь может сравнить два любых документа или две версии одного документа (например, «до» и «после» редакции агентом или исходник и его копию).

- **Backend** (`POST /api/documents/compare`) строит построчный diff по LCS и возвращает операции `equal` / `insert` / `delete` / `replace` с индексацией строк, сводку изменений (добавлено / удалено / изменено / без изменений) и флаг `truncated` с лимитом строк для очень больших файлов.
- **Версии** (`GET /api/documents/{id}/versions`) возвращают всю цепочку: исходный файл и все созданные/отредактированные копии, связанные через `source_file_id`.
- **Frontend** (`CompareViewer`) отображает результаты на уровне слов — внутри изменённых строк размечаются добавленные и удалённые слова — в двухколоночной таблице рядом.

## UI: документы и чаты

- Меню действий «⋯» у документа: **Скачать файл**, **Сравнить**, **Удалить файл**.
- Меню действий «⋯» у чата: **Переименовать**, **Удалить чат**.
- На мобильных экранах зона загрузки сжимается до компактной строки «Загрузить документы», освобождая место для чатов и списка файлов.
